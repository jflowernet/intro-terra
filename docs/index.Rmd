...
We can see the crs (coord. ref.) is GDA94 / Australian Albers (EPSG:3577). Lets get some more detail using the `crs()` function which queries the crs:
...
ras_near <- project(ras, proj_moll, meth = "near")
ras_bilinear <- project(ras, proj_moll)
ras_average <- project(ras, proj_moll, method = "average")

pal <- hcl.colors(n = ncell(ras_near), "RdYlGn")
...
par(mfrow = c(2,2))
plot(ras, main = "Original raster", col = pal)
...
We have been looking at data within the Great Barrier Reef Marine Park boundary. What if we want to extract data for several smaller areas within that area? For example we might want to know the average temperature within habitat protection zones we loaded. We could crop and mask our data for each of our zones, but this could get messy if we have a lot of zones. Happily there is an easier way.

First lets plot of zones data to see what it looks like. There are lots of columns in the data, but lets use the "PERMIT_DESC" column, which is the zone location, to map them since it has distinct and interesting descriptions for them.
...
Now, lets find the average temperature of the raster cells within our zones. We can use a function called `zonal()` and just give it our raster, followed by our zones polygons, and tell it we want the mean value.

```{r}
zonal(sst_cropped_masked_projected, zones, fun = "mean")
```

Each row shows the mean temperature for a polygon, in the same order as our `zones` object. The northern zone has the highest temperature because it is nearer the equator.

We can get other summary values, for example the maximum temperature in our zones:

```{r}
zonal(sst_cropped_masked_projected, zones, fun = "max")
```

You can get use other functions, such as min", sum", "isNA", and "notNA", and you can also use your own function. See `?zonal` for more details.

We might also want to get all the raster values in our zones. We can do this using the `extract` function:

```{r}
zones_data_df <- extract(sst_cropped_masked_projected, zones) #the raster we are extracting from first, the polygons we want data for is second

head(zones_data_df)
```

We get a data frame with where each row has the value of raser cell in our zones, and the column `ID` is used to identify each zone; 1 and 2 in our case.

Now we have a data frame of values we can, for example, do a boxplot of temperatures in each zone. I use the base R function here, but you can of course use `ggplot` or any other package you like.
...
Now everything is in the same projection, we can get the monthy temperature in each of our zones:
